...
export type Props = {
  data: Array<any>,
  numColumns: number,
  renderItem: ({ item: any, index: number, columns: number }) => React.ReactElement<any, >,
...
const _stateFromProps = ({ numColumns, data, getHeightForItem }) => {
  ...
  data.forEach((item, index) => {
    ...
    const column = columns.reduce((prev, cur) =>
      (cur.totalHeight < prev.totalHeight ? cur : prev), columns[0])
...
class FakeScrollView extends Component<{ style?: any, children?: any }> {
  render() {
    return (
...
class MasonryList extends Component<Props, State> {
  static defaultProps = {
  ...
  _renderScrollComponent = () => <FakeScrollView style={{ flex: 1 }}/>;
            ...
            return (
              <VirtualizedList
                ...
                getItemLayout={(data, i) =>
                  this._getItemLayout(index, i)}
                renderItem={({ item, i }) =>
                  renderItem({ item, i, column: index })}
